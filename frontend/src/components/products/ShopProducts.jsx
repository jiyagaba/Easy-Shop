    ...
    <div className="w-full max-w-full">
      ...
      >
        {products.map((product) => (
          <div
            key={product.id}
            className={`bg-white rounded-2xl shadow-md hover:shadow-xl transition-transform duration-500 overflow-hidden ${styles === 'grid' ? 'flex flex-col h-full' : 'flex flex-col md:flex-row'}`}
            style={styles === 'grid' ? { height: '100%' } : {}}
          >
            {/* Image Section */}
            <div
            ...
            {/* Product Details */}
            <div className={`${styles === 'grid' ? 'p-4 flex flex-col justify-between flex-grow' : 'p-4 flex flex-col justify-between md:w-2/3 w-full'}`}>
